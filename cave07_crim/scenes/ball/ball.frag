...
float map(vec3 p){
    float pl = p.y - water_level + .05 * noise(vec3(p.xz * 5, time));
...
vec3 diffuse(inout vec3 ro, inout vec3 rd, in hit h){
    ro += rd * h.t;
    rd = getSampleBiased(reflect(rd,h.n), 5, seed);
    // rd = reflect(rd, h.n);
    seed += vec2(1.13756, 1.37683);

    return vec3(.9) * step(fract(abs(ro.x) + abs(ro.z) - time), .8);
}
...
void main(void)
{
    ...
    float anim = 0.5 + 0.5 * cos(PI * exp(-5 * buttons[4].y));
    // ro.xz *= r2d(PI * (buttons[4].w + anim));
    ro.xz *= r2d(.1 * time);
