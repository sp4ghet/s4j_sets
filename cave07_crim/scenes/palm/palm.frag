...
float funky = 0.5 + 0.5 * cos(PI * exp(-5. * buttons[4].y));
mat3 rot = euler(PI * .13, PI * .27, PI * .17 + funky);
...
vec4 map(vec3 q){
    ...
    float thing = pillar(p, .25, .5) / 2;
    chmin(d, vec4(thing, 0,0,0));
...
void main(void)
{
    ...
    vec3 fog = mix(vec3(.9, .2, .7), vec3(.8, .7, .4), rd.y);
    c = mix(fog, c, exp(-.01 * t));
